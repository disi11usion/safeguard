...
const StocksTable = () => {
  ...
  const fetchStocksData = async () => {
    try {
      setLoading(true);
      // Backend automatically uses mock data based on USING_MOCK_DATA env variable
      const response = await apiService.getComprehensiveMarketData('stocks');
  ...
  const handleRowClick = (stock) => {
    // Backend automatically handles mock data mode
    navigate(`/analysis/ticker?symbol=${stock.symbol}&name=${encodeURIComponent(stock.name)}&market=stocks`);
  };
            ...
            <AnimatePresence>
              {filteredData.map((stock) => (
                ...
                >
                  ...
                  <TableCell>
                    {stock.change_percent === "N/A" || !stock.change_percent ? (
                      <span className="text-muted-foreground">N/A</span>
                    ) : (
                      <div className={`flex items-center gap-2 font-medium ${
                        typeof stock.change_percent === 'string' && stock.change_percent.includes('-') 
                          ? 'text-red-500' 
                          : 'text-green-500'
                      }`}>
                        {typeof stock.change_percent === 'string' && stock.change_percent.includes('-') ? (
                  ...
                  <TableCell className="font-medium text-foreground">
                    <div className="flex flex-col text-sm">
                      ...
                      <span className="text-muted-foreground">{formatPrice(stock.close)}</span>
                    </div>
                  </TableCell>
                  <TableCell className="font-medium text-foreground">
                    <div className="flex flex-col text-sm">
                      <span className="text-green-500">{formatPrice(stock.high)}</span>
                      <span className="text-red-500">{formatPrice(stock.low)}</span>
                    </div>
                  </TableCell>
